...
//	__gui_draw_rect(x, y, lh->bbox_w, lh->bbox_h, GUI_COLOR_YELLOW, 0);
	...
	gui_draw_text(lh->text, xx, y, lh->font, lh->color);
}
...
static void switch_update_layout(switch_t *sw)
{
	...
	if (sw->font && sw->text[0])
		gui_get_text_sizes(sw->text, sw->font, &tw, &th);
...
void draw_switch(switch_t *sw)
{
	...
	/* подпись */
	if (sw->cap_w && sw->font)
		gui_draw_text(sw->text, win->x1 + sw->x + sw->cap_x, win->y1 + sw->y + sw->cap_y, sw->font, GUI_COLOR_WHITE);
...
void button_update_text_metrics(button_t *bh)
{
	...
	if (sep) {
		...
		tmp[n1] = '\0';
		int w1 = 0, h1 = 0;
		gui_get_text_sizes(tmp, bh->font, &w1, &h1);
		bh->line1_w = (uint16_t) w1;
		int w2 = 0, h2 = 0;
		gui_get_text_sizes(sep + 1, bh->font, &w2, &h2);
		bh->line2_w = (uint16_t) w2;
	} else {
		bh->sep_pos = 0;
		int w = 0, h = 0;
		gui_get_text_sizes(bh->text, bh->font, &w, &h);
...
void draw_button(button_t *bh)
{
	...
	const int line_h = gui_get_font_height(bh->font);

	if (!bh->sep_pos) {
		gui_draw_text(bh->text, shiftX + x + (bh->w - bh->line1_w) / 2, shiftY + y + (bh->h - line_h) / 2, bh->font, textcolor);
	} else {
		char tmp[TEXT_ARRAY_SIZE];
		int n1 = bh->sep_pos < TEXT_ARRAY_SIZE - 1 ? bh->sep_pos : TEXT_ARRAY_SIZE - 1;
		memcpy(tmp, bh->text, n1);
		...
		gui_draw_text(tmp, shiftX + x + (bh->w - bh->line1_w) / 2, y_start, bh->font, textcolor);
		gui_draw_text(bh->text + bh->sep_pos + 1, shiftX + x + (bh->w - bh->line2_w) / 2, y_start + line_h, bh->font, textcolor);
	}
	if (bh->is_focus)
...
void draw_canvas(canvas_t *ca)
{
	...
	if (ca->background)
		__gui_draw_semitransparent_rect(x, y, ca->w - 1, ca->h - 1,
				ca->color, alpha);
...
void gui_canvas_print(const char *text, int x, int y, gui_font_t *font, gui_color_t color)
{
	...
	gui_draw_text(text, win->draw_x1 + ca->x + x, win->draw_y1 + ca->y + y,
			font, color);
}
...
void textfield_update_size(text_field_t *tf)
{
    ...
    gui_get_text_sizes("M", tf->font, &w_char, &h_char);
...
void draw_textfield(text_field_t *tf)
{
    ...
    int j = tf->index - 1;
    int line_h = gui_get_font_height(tf->font);
...
//    __gui_draw_rect(x, y, tf->w, tf->h, GUI_COLOR_WHITE, 0);
    ...
    for (uint8_t i = 0; i < tf->h_str; i ++, j --)
    {
        ...
        if (strlen(tf->string[j].text))
        	gui_draw_text(tf->string[j].text, x, y + line_h * pos, tf->font, tf->string[j].color_line);
    }
}

// *************** Sliders ****************

static void slider_update(slider_t *sl)
{
...
// определяется ПРЕФИКСОМ имени 'name' (btn_ / lbl_ / sl_ / ta_ / tf_ / sw_ / ca_).
//
...
//                           быть включены одновременно (проверка в calculate_window_position).
//
// TYPE_TEXT_FIELD (tf_):    uint32_t w_sim, uint32_t h_str, uint32_t direction
...
//
// TYPE_CANVAS (ca_):        int w, int h
//                           w, h - размеры холста в пикселях (координаты
//                                  относительно области отрисовки draw_x1/draw_y1 окна).
//                           Примечание: цвет, прозрачность, рамка и фон по умолчанию
//                           инициализируются стандартными значениями и изменяются
//                           через gui_obj_set_prop (GUI_OBJ_COLOR, GUI_OBJ_BORDER, GUI_OBJ_BACK).
uint8_t gui_obj_create(const char *name, ...)
{
	...
	case TYPE_LABEL:
	{
		...
		lh->width = lh->width > TEXT_ARRAY_SIZE ? TEXT_ARRAY_SIZE : lh->width;
		lh->font_size = gui_sizes.labels_font_size;
		lh->bbox_align = ALIGNMENT_LEFT;
		lh->font_owned = 0;

		lh->font = gui_get_label_font();
		memset(lh->text, '0', lh->width);		// для расчёта bbox
		lh->text[lh->width] = '\0';
		gui_get_text_sizes(lh->text, lh->font, &lh->bbox_w, &lh->bbox_h);
		lh->width_text_pix = lh->bbox_w;
		lh->baseline = gui_get_font_ascent(lh->font);
	...
	case TYPE_TEXT_FIELD:
	{
		...
		tf->font = gui_get_label_font(); 		// Переиспользуем моноширинный шрифт меток
	...
	case TYPE_LABEL:
	{
		...
		if (prop == GUI_OBJ_VISIBLE) lh->visible = !! va_arg(arg, int);
            ...
            if (strcmp(tmp, lh->text) != 0) {
                ...
                gui_invalidate_text(lh->text, lh->font);
#endif /* ! GUI_USE_PORT */
			...
			if (lh->font_owned && lh->font) {
				gui_close_font(lh->font);
				lh->font = NULL;
			}
			char *path = va_arg(arg, char *);
			lh->font_size = va_arg(arg, int);
			lh->font = gui_open_font(path, lh->font_size);
			lh->font_owned = 1;
			if (!lh->font) {
				printf("[GUI] Failed to open font %s size %d\n", path, lh->font_size);
				lh->font = gui_get_label_font();
				lh->font_owned = 0;
			}
		}
        ...
        {
            int w, h;
            gui_get_text_sizes(lh->text, lh->font, &w, &h);
            ...
            if (prop == GUI_OBJ_FONT) {
                ...
                widest[lh->width] = '\0';
                int ww, wh;
                gui_get_text_sizes(widest, lh->font, &ww, &wh);
                lh->bbox_w = ww;
            }
        }
		break;
...
/* Применение выравнивания к объекту относительно прямоугольника (x2, y2, w2, h2).
   baseline2 — базовая линия текста опорного объекта (только для TYPE_LABEL,
   используется при ALIGN_RIGHT_DOWN для вертикального совмещения по тексту;
   для остальных типов объектов и для областей массового выравнивания = 0). */
...
// Выравнивание однотипных объектов (кнопка, метка, слайдер, переключатель, canvas)
// в сетке, начиная с объекта 'name' и далее по возрастанию индекса в окне.
// Объекты берутся из того же массива окна, что и 'name', без явной передачи имён.
...
// Выравнивание однотипных объектов (кнопка, метка, слайдер, переключатель, canvas)
// в сетке по именам, переданным в массиве 'names'.
// Обработка по возрастанию индекса (в порядке создания).
// Все объекты должны быть одного типа (определяется по префиксу имени первого элемента).
