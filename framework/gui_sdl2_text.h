...
/* Отрисовка текста с цветовой разметкой (/csRRGGBB ... /cd /ce).
   Если в тексте нет маркеров — эквивалентно gui_sdl2_draw_text. */
void gui_sdl2_draw_text_colored(const char *text, int x, int y,
                                TTF_Font *font, gui_color_t default_color);

/* Ширина текста без учёта маркеров разметки (символы /cs.../cd/ce не считаются).
   Если в тексте нет маркеров — эквивалентно gui_sdl2_get_text_size. */
void gui_sdl2_get_text_size_colored(const char *text, TTF_Font *font,
                                    int *w, int *h);

/* Возвращает 1, если в строке присутствует хотя бы один маркер /cs,
   иначе 0 (быстрый путь для проверки необходимости цветной отрисовки). */
